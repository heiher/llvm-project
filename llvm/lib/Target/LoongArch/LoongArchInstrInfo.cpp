...
void LoongArchInstrInfo::insertIndirectBranch(MachineBasicBlock &MBB,
  ...
  MachineInstr *PCAXXU12I = nullptr;
  MachineInstr *ADDI = nullptr;
  auto II = MBB.end();

  if (STI.is64Bit()) {
    PCAXXU12I = BuildMI(MBB, II, DL, get(LoongArch::PCALAU12I), ScratchReg)
                    .addMBB(&DestBB, LoongArchII::MO_PCREL_HI);
    ADDI = BuildMI(MBB, II, DL, get(LoongArch::ADDI_D), ScratchReg)
               .addReg(ScratchReg)
               .addMBB(&DestBB, LoongArchII::MO_PCREL_LO);
  } else {
    MCSymbol *PCAddSymbol = MF->getContext().createNamedTempSymbol("pcadd_hi");
    PCAXXU12I = BuildMI(MBB, II, DL, get(LoongArch::PCADDU12I), ScratchReg)
                    .addMBB(&DestBB, LoongArchII::MO_PCADD_HI);
    PCAXXU12I->setPreInstrSymbol(*MF, PCAddSymbol);
    ADDI = BuildMI(MBB, II, DL, get(LoongArch::ADDI_W), ScratchReg)
               .addReg(ScratchReg)
  ...
  Register Scav = RS->scavengeRegisterBackwards(
      LoongArch::GPRRegClass, PCAXXU12I->getIterator(), /*RestoreAfter=*/false,
  ...
  if (Scav != LoongArch::NoRegister)
    ...
    storeRegToStackSlot(MBB, PCAXXU12I, Scav, /*IsKill=*/true, FrameIndex,
                        &LoongArch::GPRRegClass, TRI, Register());
    TRI->eliminateFrameIndex(std::prev(PCAXXU12I->getIterator()),
                             /*SpAdj=*/0, /*FIOperandNum=*/1);
    PCAXXU12I->getOperand(1).setMBB(&RestoreBB);
    if (STI.is64Bit())
